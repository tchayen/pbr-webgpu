...
export function Trigger({
  ...
  return (
    <SelectPrimitive.Trigger
      className={twMerge(
        "flex h-6 min-w-0 items-center justify-between rounded bg-slatedark1 px-2 py-2 outline-none focus-visible:ring-1 focus-visible:ring-bluedark8 focus-visible:ring-offset-0 disabled:cursor-not-allowed disabled:opacity-50",
        className,
      )}
      {...props}
    >
    ...
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        className={twMerge(
          ...
          "relative z-50 overflow-hidden rounded bg-slatedark1 text-slatedark12 shadow-lg data-[side=bottom]:translate-y-1 data-[side=left]:-translate-x-1 data-[side=right]:translate-x-1 data-[side=top]:-translate-y-1",
          className,
        )}
        position="popper"
        {...props}
